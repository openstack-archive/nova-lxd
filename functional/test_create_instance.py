...
class LXDTestServerActions(FunctionalTestCase):
    ...
    def test_create_delete_instnace(self):
        self.create_image('lxd')
        instance_name = uuid.uuid1()
        instance = self.create_instance('lxd', instance_name)
        self.delete_instance(instance.id)
    ...
    def test_create_instance_invalid_qcow2_image(self):
    ...
    def test_create_instance_with_ephmeral_storage(self):
        self.create_image('lxd')
        instance_name = uuid.uuid1()
        instance = self.create_instance('lxd', instance_name, flavor='m1.lxc')
        output = self.run_command(instance['accessIPv4'], 'mount')
        self.assertTrue('ephemeral' in output)
        self.cloud.delete_server(instance_name)
