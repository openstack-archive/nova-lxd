...
class LXDDriver(driver.ComputeDriver):
    ...
    def get_available_resource(self, nodename):
        ...
        if storage_driver == 'zfs':
            ...
            # key from the config.  So, if it fails, we need to grab the first
            # storage pool and use that as the name instead.
            try:
                pool_name = lxd_config['config']['storage.zfs_pool_name']
            except KeyError:
                pool_name = self.client.storage_pools.all()[0].name
