...
def _root(instance, client, *_):
    ...
    # The multiple int casts below are handling a cast of the final division
    # result, while the second cast is casting the specs[] content to an int
    # so that we can use it in division
    ...
    for vifaddr in network_info:
        ...
        if vif_inbound_limit:
            devices[key]['limits.ingress'] = '{}Mbit'.format(
                int(vif_inbound_limit * units.k * 8 / units.M))
        ...
        if vif_outbound_limit:
            devices[key]['limits.egress'] = '{}Mbit'.format(
                int(vif_outbound_limit * units.k * 8 / units.M))
    return devices
