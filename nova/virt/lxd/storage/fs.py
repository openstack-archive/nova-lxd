...
class LXDFSStorageDriver(storage.LXDStorageDriver):
    ...
    def create_storage(self, block_device_info, instance):
        ...
        if ephemeral_config:
            ...
            fileutils.ensure_tree(ephemeral_config['src_dir'])
            utils.execute(
    ...
    def remove_storage(self, block_device_info, instance):
        ...
        ephemeral_config = volumeops.ensure_ephemeral(
            block_device_info, instance)
        if ephemeral_config:
            if os.path.exists(ephemeral_config['src_dir']):
                utils.execute(
                    'chown', '-R', '%s:%s' % (os.getuid(), os.getuid()),
                    ephemeral_config['src_dir'], run_as_root=True)
                shutil.rmtree(ephemeral_config['src_dir'])

    def resize_storage(self, block_device_info, instanace):
