...
def _add_ovs_bridge_port(bridge, dev):
    utils.execute('ovs-vsctl', '--', '--if-exists', 'del-port', dev,
                  '--', 'add-port', bridge, dev, run_as_root=True)
...
def _post_plug_wiring_veth_and_bridge(vif):
    ...
    if not linux_net.device_exists(v1_name):
        ...
        if vif['type'] == 'ovs' and (_is_no_op_firewall() or
                                     not vif.is_hybrid_plug_enabled()):
            # NOTE(jamespage): wire tap device directly to ovs bridge
            _add_ovs_bridge_port(config['bridge'], v1_name)
        else:
            # NOTE(jamespage): wire tap device linux bridge
            _add_bridge_port(config['bridge'], v1_name)
    else:
        for dev in [v1_name, v2_name]:
            linux_net._set_device_mtu(dev, mtu)
