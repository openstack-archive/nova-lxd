...
def _post_plug_wiring_bridge(vif):
    config = get_config(vif)
    network = vif.get('network')
    mtu = network.get_meta('mtu') if network else None
    v1_name = get_vif_devname(vif)
...
# VIF_TYPE_TAP = 'tap'
def _post_plug_wiring_tap(vif):
    pass


POST_PLUG_WIRING = {
    'bridge': _post_plug_wiring_bridge,
    'ovs': _post_plug_wiring_ovs,
    'tap': _post_plug_wiring_tap,
}
